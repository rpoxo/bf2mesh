...
# should be a object method actually
def get_vertex_copy_with_offset(vmesh, offset):
    ...
    for vertice in vmesh.vertices_attributes:
        ...
        new_position = (
            new_position_x,
            vertice_clone['position'][1],
            vertice_clone['position'][2])
...
def is_vertex_similar(vert1, vert2):
    for attribute in vert1.keys():
        ...
        if vert1[attribute] != vert2[attribute]:
            return False
    return True
            ...
            if vertex['position'] == similar_vertex['position']:
                if is_vertex_similar(vertex, similar_vertex):
                    ...
                    for selected_vertex in new_vbo:
                        if is_vertex_similar(vertex, selected_vertex) and vertex[
                                'position'] == selected_vertex['position']:
                            is_selected = True
                            break
...
test_object_std = os.path.join(*['objects',
                                 ...
                                 'meshes',
                                 'evil_box.staticmesh'])
test_object_two_lods = os.path.join(
    *['objects', 'staticobjects', 'test', 'evil_box3', 'meshes', 'evil_box3.staticmesh'])
...
test_object_tri = os.path.join(*['objects',
                                 'staticobjects',
                                 'test',
                                 'evil_box8',
                                 'meshes',
...
path_object_container = os.path.join(
    bf2.Mod().root,
    os.path.join(
        *
        [
            ...
            'container',
            'meshes',
            'container.staticmesh']))
path_object_toilet = os.path.join(bf2.Mod().root, os.path.join(
    *['objects', 'staticobjects', 'pr', 'toilet', 'meshes', 'toilet.staticmesh']))
...
for index, new_vertex in enumerate(
    get_vertex_copy_with_offset(
        vmesh_std, 1.0)):
    print(
        'new[{}] {} {}'.format(
            index +
...
print('vmesh_std.geoms[0].lod[0].mat[0].inum = {}'.format(
    vmesh_std.geoms[0].lod[0].mat[0].inum))
print('vmesh_std.geoms[0].lod[0].mat[0].vnum = {}'.format(
    vmesh_std.geoms[0].lod[0].mat[0].vnum))
...
vmesh_std.geoms[0].lod[0].mat[
    0].inum = new_inum_per_mat * 2  # seems to be .indexnum
vmesh_std.geoms[0].lod[0].mat[
    0].vnum = new_vnum_per_mat * 2  # == len(vertices)
...
vmesh_std.geoms[0].lod[1].mat[
    0].inum = new_inum_per_mat  # seems to be .indexnum
