...
def remove_vertice_id(vmesh, id):
    ...
    print('new len = {}'.format(vmesh.vertnum))
...
def remove_attribute(vmesh, attrib_to_remove):
    ...
    for attrib_id, attrib in enumerate(vmesh.vertattrib):
        ...
        if offset_attrib != 0 and offset != 0:
            ...
            print('MOVING {} offset from {} to {} by {}'.format(usage, attrib.offset, new_attrib_offset, offset_attrib))
    ...
    if attrib_id_to_remove !=0:
        ...
        print('REDUCING STRIDE from {} to {} by {}'.format(vmesh.vertstride, vmesh.vertstride - reduce_stride_by, reduce_stride_by))
        ...
        print('new vertices len = {} after {} removed'.format(len(vmesh.vertices), total_data_removed))
...
def edit_vertex(vmesh, vid, vattribute, vdata):
    for attrib_id, attrib in enumerate(vmesh.vertattrib):
        ...
        if usage == vattribute:
            print('SETTING DATA for [{}]{}'.format(vid, vattribute))
            for vertid in range(vmesh.vertnum-1, -1, -1):
...
vmesh.write_file_data('generated/generated_box/meshes/generated_box_edit.staticmesh')
