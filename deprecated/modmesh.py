...
class bf2lod:
    ...
    def __init__(self):
        ...
        # apparently those a geomPart objects for animating sprints\rotbundles
        # etc
        self.nodenum = None
        self.nodes = []  # matrix4
...
class VisMeshTransform:
    ...
    def delete_geom_id(self, id_delete):
        ...
        # remove vertex data
        vstart = int(
            self.vmesh.vertstride /
            self.vmesh.vertformat *
            self.vmesh.geoms[id_delete].lods[0].materials[0].vstart)
    ...
    def order_geoms_by(self, order):
        # would be better to implement actual geom data reordering instead of
        # geom swap...
    ...
    def rename_texture(self, geom, lod, material, map, path):
        self.vmesh.geoms[geom].lods[lod].materials[
...
class VisMesh(object):
    ...
    def get_vertex(self, id_vertex):
        ...
        for attrib in self.vertattrib:
            ...
            if flag == 255:
                continue
            if vartype == D3DDECLTYPE.UNUSED:
                continue
            if vartype.name in vertex.__dict__:
                raise
    ...
    def update_boundaries(self):
        ...
        for geom in self.geoms:
            for lod in geom.lods:
                ...
                for material in lod.materials:
                    material_vertices = verts[
                        material.vstart:material.vstart + material.vnum]
                    ...
                    for vertex in material_vertices:
                        for id_axis, axis in enumerate(material.mmin):
                            if vertex.POSITION[
                                    id_axis] < material.mmin[id_axis]:
                                material.mmin[
                                    id_axis] = vertex.POSITION[id_axis]
    ...
    def update_vertices(self, vertex_objects):
        ...
        self.vertnum = int(len(self.vertices) /
                           (self.vertstride / self.vertformat))
        ...
        for geom in self.geoms:
            for lod in geom.lods:
                for mat in lod.materials:
                    for vertex in vertobjects[
                            mat.vstart:mat.vstart + mat.vnum]:
                        position = getattr(vertex, D3DDECLUSAGE.POSITION.name)
                        new_position = tuple(
                            [sum(_) for _ in zip(*[position, offset])])
                        setattr(
                            vertex,
        ...
        for id_geom, geom in enumerate(self.geoms):
            for id_lod, lod, in enumerate(geom.lods):
                for id_material, material in enumerate(lod.materials):
                    othermat = other.geoms[id_geom].lods[
                        id_lod].materials[id_material]
                    material_vertices = []
                    material_indices = []
                    ...
                    for vertex in verts2[
                            othermat.vstart:othermat.vstart + othermat.vnum]:
                        material_vertices.append(vertex)

                    for idx, index in enumerate(
                            self.index[material.istart:material.istart + material.inum]):
                        material_indices.append(index)
                    for idx, index in enumerate(
                            other.index[othermat.istart:othermat.istart + othermat.inum]):
    ...
    def _read_vertex_block(self, fo):
        ...
        self.vertices = struct.Struct(fmt).unpack(
            fo.read(size))  # TODO: remove conversions everywhere
        #print('>> {}'.format(fo.tell()))
    ...
    def _read_index_block(self, fo):
        ...
        self.index = struct.Struct(fmt).unpack(
            fo.read(size))  # TODO: remove conversions everywhere
