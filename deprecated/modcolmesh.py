...
class bf2colface(object):
    ...
    def __init__(self, v1=0, v2=0, v3=0, m=0):
        # those are vertex indices + material
        self.v1 = v1
        self.v2 = v2
        self.v3 = v3
        self.material = m
...
class bf2collod(object):
    ...
    def __init__(self):
        ...
        self.vertnum = 0
         # list of Vec3
        self.vertices = []
        # some unknown 2 bytes ints, seems to be always zeroes
        self.vertids = []
        ...
        self.anum = 0
         # definitely not face indices, can be negative for single tri
        self.adata = []
    ...
    def merge(self, other):
        ...
        self.vertids.extend(other.vertids)

        # updating faces
        for face in other.faces:
            face.v1 += other.vertnum
            face.v2 += other.vertnum
            face.v3 += other.vertnum

            self.faces.append(face)
        self.facenum += other.facenum

        # updating boundaries
        for vertex in self.vertices:
            for key, value in vertex.__dict__.items():
                if key not in ['x', 'y', 'z']: continue
                if getattr(self.min, key) > getattr(vertex, key):
                    setattr(self.min, key, value)
                if getattr(self.max, key) < getattr(vertex, key):
                    setattr(self.max, key, value)

                if getattr(self.bmin, key) > getattr(vertex, key):
                    setattr(self.bmin, key, value)
                if getattr(self.bmax, key) < getattr(vertex, key):
                    setattr(self.bmax, key, value)
                

        # updating unknowns
        self.ydata.extend(other.ydata)
        self.ynum += other.ynum

        self.zdata.extend(other.zdata)
        self.znum += other.znum

        self.adata.extend(other.adata)
        self.anum += other.anum
...
class bf2colsubgeom(object):
    ...
    def merge(self, other):
        for id, lod in enumerate(other.lods):
            self.lods[id].merge(lod)
...
class bf2colgeom(object):
    ...
    def merge(self, other):
        for id, subgeom in enumerate(other.subgeoms):
            self.subgeoms[id].merge(subgeom)
...
class ColMesh(object):
    ...
    def merge(self, other):
        for id, geom in enumerate(other.geoms):
            self.geoms[id].merge(geom)
            ...
            for subgeom in geom.subgeoms:
                for lod in subgeom.lods:
                    for id_vert, vertex in enumerate(lod.vertices):
                        lod.vertices[id_vert] = vertex + offset
